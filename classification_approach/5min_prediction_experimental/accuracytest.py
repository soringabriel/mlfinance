...
for x in range(len(predictions)):
    ...
    pac = predictions[x][past_data_size + 6]
    ...
    acc_pac.append(1 if pac == classification else 0)
    sum_correct = acc_knn_30[-1] + acc_knn_max[-1] + acc_svm[-1] + acc_naive_bayes[-1] + acc_decision_tree[-1] + acc_rf[-1] + acc_pac[-1]
    acc_max.append(1 if sum_correct > 3 else 0)
    print(data[x][0],predictions[x][0],classification,knn_30,knn_max,svm,naive_bayes,decision_tree,rf,pac,sum_correct)
...
print("Accuracy for pac", sum(acc_pac) / len(acc_pac))
